    ...
    <div class="col-lg-8"> 
        <div class="login-form">
            ...
            @if(\Session::get('success'))
                ...
                @csrf
              <select class="input-field" name="teamType" required>
                
                <option value="Team"   {{ Request::get('authType')=='Team' ? 'selected' : '' }}>Team</option>
                <option value="Branch" {{ old('authType')=='Branch' ? 'selected' : '' }}  {{ Request::get('authType')=='Branch' ? 'selected' : '' }}>Branch</option>
                <option value="Super Admin" {{ old('authType')=='Super Admin' ? 'selected' : '' }}  {{ Request::get('authType')=='Super Admin' ? 'selected' : '' }}>Super Admin</option>
            </select>
            <input type="email" class="input-field" placeholder="Email" name="email" data-parsley-type="email" required>
            @if ($errors->has('email'))
              ...
              @endif
            <p id="messageRes"></p>
            <button type="submit" class="login-button">Send Password Rest Link</button>
        </div>
      </div>
      <div class="col-lg-4"> 
...
<script>
  ...
  $(document).ready(function() {
    $('#resetForm').on('submit', function(e) {
        ...
        $('#messageRes').empty();
           $.ajax({
                url: " {{ url('send-forgot-password') }}", 
                method: 'POST',
                data: $(this).serialize(),
                success: function(response) {
                    $('#formLoader').hide();
                    $('#messageRes').html('We have emailed your password reset link!');
